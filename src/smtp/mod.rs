...
impl Smtp {
    ...
    /// Return number of seconds since last successful connect/send
    /// operation.  None indicates that last success time is unknown.
    pub fn secs_since_last_success(&self) -> Option<u64> {
        if let Some(last_success) = self.last_success {
            Some(Instant::now().duration_since(last_success))
        }
